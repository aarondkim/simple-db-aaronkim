...
public class JoinOptimizer {
    ...
    public double estimateJoinCost(LogicalJoinNode j, int card1, int card2,
        ...
        if (j instanceof LogicalSubplanJoinNode) {
            ...
            // The IO and CPU cost for a nest loop join algorithm
    ...
    public int estimateJoinCardinality(LogicalJoinNode j, int card1, int card2,
        ...
        if(j.p == Predicate.Op.EQUALS){
            if(t1pkey && t2pkey){
                // Joins on primary key of two relations, choosing cardinality of
                // the smaller of the two tables
                return Math.min(card1, card2);
            }
            // For equality joins when one of the attributes is a primary key,
            // number of tuples produced by the join cannot be larger than the
            // cardinality of the non-primary key attribute
                ...
                // Choosing cardinality of the larger of the two tables for
                // no primary key join
                return Math.max(card1, card2);
            }
        } else if(j.p == Predicate.Op.NOT_EQUALS){
            // Not equals condition could potentially be huge cardinality since
            // it is like the opposite of point selection. Opted for cartesian product
            return card1 * card2;
        } else {
            // For range scans, assumed that a fixed fraction of the cross-product is emitted
            // by range scans (30% in this case)
